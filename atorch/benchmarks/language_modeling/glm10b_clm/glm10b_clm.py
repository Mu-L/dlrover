...
from transformers import MODEL_MAPPING  # noqa: E402
...
def execute_nvidia_smi(timeout=5):
    subprocess.run(["nvidia-smi"], timeout=timeout)
...
def main():
    ...
    with init_empty_weights_with_disk_offload(ignore_tie_weights=False):
        ...
        print("number of parameters of this model:", num_params)
    ...
    try:
        from transformers_modules.local.modeling_glm import GLMBlock
    except ImportError as e:
        GLMBlock = None
        print(f"Failed to import GLMBlock, error: {e}")
        raise e
    fsdp_config = {
        "sync_module_states": True,
    }
    if GLMBlock is not None:
    ...
    status, result, best_strategy = auto_accelerate(
        ...
        ignore_dryrun_on_load_strategy=args.skip_atorch_autoacc_dryrun,
    )
    ...
    # Optimizer will be created by auto accelerate
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            loss = outputs.loss
            losses.append(gather(loss.repeat(args.per_device_eval_batch_size)))
    ...
    if is_main_process():
        print(
            "{} epoches has been trained in {:.3f} s with total_batch_size = {} using {} GPU(s)".format(
                args.num_train_epochs, training_time, args.total_train_batch_size, atorch.world_size()
            )
        )
        print(
            "Training throughput is {:.3f} epoch/s, {:.3f} samples/s, {:.3f} it/s ".format(
                args.num_train_epochs / training_time,
                args.num_train_epochs * len(train_dataset) / training_time,
    ...
    if args.output_dir is not None:
        ...
        unwrapped_model.save_pretrained(args.output_dir, is_main_process=is_main_process(), save_function=torch.save)
        if is_main_process():
            tokenizer.save_pretrained(args.output_dir)
            # if args.push_to_hub:
